...
template<typename T_numFrames, typename T_numThreads,typename T_ThreadSpec>
struct BlockDynSharedMemBytes<StencilKernel2, alpaka::onHost::FrameSpec<T_numFrames, T_numThreads,T_ThreadSpec>>
{
    BlockDynSharedMemBytes(StencilKernel2 const&, alpaka::onHost::FrameSpec<T_numFrames, T_numThreads,T_ThreadSpec> spec)
        : spec_(spec)
    {
    }
    ...
    alpaka::onHost::FrameSpec<T_numFrames, T_numThreads,T_ThreadSpec> spec_;
};
*/
...
auto example( auto const deviceSpec,
        auto const computeExec, uint32_t i) -> int
{
    using namespace alpaka;
    ...
    */
    ...
    static auto tuningSession = getSessionFromExec(computeExec, toRTime, devAcc, numNodes);
    ...
    {
#ifdef Debug
        std::cout << "[Iteration]" << "," << step << "," << "Stencil" << std::endl;
#endif
        auto startTime_iN = std::chrono::high_resolution_clock::now();
...
#ifdef Debug
        alpakaEvents[step] = alpaka::tune::global::timingAccessor();
        std::chrono::duration<double> elapsedTime_IN = endTime_IN - startTime_iN;
        auto ns_count_ = std::chrono::duration_cast<std::chrono::nanoseconds>(endTime_IN - startTime_iN).count();
        tunerEvents[step] = ns_count_;
