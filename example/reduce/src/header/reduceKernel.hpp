...
ALPAKA_FN_INLINE ALPAKA_FN_ACC T dynamicReduce(
...
ALPAKA_FN_INLINE ALPAKA_FN_ACC auto accFunctor(operationType const& type, Buftype& buf, IndexType index) -> Td
{
    using chunkType = mystructa<Td, strideSize>;

    chunkType const* alignedBuf = reinterpret_cast<chunkType const*>(buf.data()) + (index / strideSize);
    ...
    // return reduce::operate<operationType>(reduce::operate<operationType>(elements[0],
    // elements[1]),reduce::operate<operationType>(elements[2], elements[3]));//this is now hardcoded with a stride of
    // 4
}
    ...
    std::index_sequence<Indices...>)
{
    ...
    float_t results[sizeof...(Indices)] = {accFunctor<stride, float_t>(type, dataBuf, advance(iter, Indices))...};

    // Perform hierarchical reduction on the results array
    return dynamicReduce(type, results, std::make_index_sequence<sizeof...(Indices)>{});
}
...
public:
        ...
        auto const end) const -> void
    {
        ...
        auto traverseOverBlocks
            = onAcc::makeIdxMap(acc, onAcc::worker::blocksInGrid, IdxRange{IdxVec{0}, blockDataExtent, frameExtent});
        ...
        for(auto [DataElemIdxInFrame] : onAcc::makeIdxMap(
                ...
                IdxRange{acc[layer::thread].count(), frameExtent.x()}))
        {
            sdata[acc[layer::thread].idx().x()]
                = reduce::operate(type, sdata[acc[layer::thread].idx().x()], sdata[DataElemIdxInFrame]);
        }
