...
TEST_CASE("ConfigQueue basic behaviour with ConfigRecords", "[ConfigQueue]")
{
    ...
    auto& rec = recOpt->get();
    REQUIRE((rec.config == c1 || rec.config == c2));
    REQUIRE_FALSE(rec.fullFlag);
}
...
{
    ConfigQueue<TestRecord> queue;
    REQUIRE(queue.empty()); //dosnt work

    auto recOpt = queue.get();
    REQUIRE_FALSE(recOpt.has_value());
}
...
TEST_CASE("ConfigQueue random access and cleanup", "[ConfigQueue]")
{
    ...
    records[3].fullFlag = true;
    // records[5].fullFlag = true;

    // Trigger cleanup
    for(int i = 0; i < 1000; ++i)
        if(queue.get().has_value())
        {
            REQUIRE(!queue.get().value().get().fullFlag);
        }
