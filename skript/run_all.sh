...
for exp in 17 19 21 23 25 27 29; do
    ...
    "$BINARY_DIR/benchmark/babelstream/babelstream" --array-size=$size --number-runs=40000 > /dev/null || echo "babelstream failed for size=$size"
done
...
for factor in 1 4 8 12 16; do
    size=$((1024 * factor))
    echo "Running size=$size"
    "$BINARY_DIR/example/heatEqu2DWithFrame/heatEquation2D_Frame" --numNodes=$size > /dev/null || echo "heatEquation2D_Frame failed for size=$size"
done
