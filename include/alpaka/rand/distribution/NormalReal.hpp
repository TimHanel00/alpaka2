...
namespace alpaka::rand::distribution
{
    namespace internal
    {
        ...
        struct BoxMuller
        {
            ...
            constexpr T_FP operator()(T_Rng& rng)
            {
                ...
                // Generate two uniform floats in (0,1) -- this is equivalent to UniformReal<T_FP, interval::OO>
                // uniformDist; but doesnt use templates
                auto uniformDist = UniformReal{T_FP{0}, T_FP{1}, interval::oo};
    ...
    struct NormalReal
    {
        ...
        result_type operator()(T_Engine& engine)
        {
            return T_Impl{}(engine) *m_stdDev + m_mean;
        }
